...
for module_dir in $changedmodules; do
	...
	echo -e "Running rspec-puppet tests for module $module_path..."
	cd $module_dir
	...
	if [ $RC -ne 0 ]; then
	    cat $error_msg
	    echo -e "Error: rspec-puppet test(s) failed for $module_dir (see above)"
...
if [ "$syntax_errors" -ne 0 ]; then
    echo -e "Error: $syntax_errors rspec-puppet test(s) failed. Commit will be aborted."
    exit 1
fi
